...
func (be BytecodeExp) run(c *Char) BytecodeValue {
	...
	for i := 1; i <= len(be); i++ {
		...
		case OC_p2:
			...
			i += int(*(*int32)(unsafe.Pointer(&be[i]))) + 4
		case OC_stateowner:
			if c = sys.getChar(c.ss.sb.playerNo, 0); c != nil {
				i += 4
				continue
			}
			sys.bcStack.Push(BytecodeSF())
		...
		case OC_pos_x:
			...
			sys.bcStack.PushF(((c.pos[0]+bindVelx)*(c.localscl/oc.localscl) - sys.gs.cam.Pos[0]/oc.localscl))
		case OC_pos_y:
...
func (b StateBlock) Run(c *Char, ps []int32) (changeState bool) {
	...
	//sys.workingChar = c
	sys.workingChar = sys.getChar(c.ss.sb.playerNo, 0)
	if b.loopBlock {
		if b.forLoop {
			if b.forAssign {
...
func (sc posSet) Run(c *Char, _ []int32) bool {
	...
	StateControllerBase(sc).run(c, func(id byte, exp []BytecodeExp) bool {
		switch id {
		case posSet_x:
			x := sys.gs.cam.Pos[0]/crun.localscl + exp[0].evalF(c)*lclscround
			crun.setX(x)
...
func (sc explod) Run(c *Char, _ []int32) bool {
	...
	StateControllerBase(sc).run(c, func(id byte, exp []BytecodeExp) bool {
		...
		case explod_anim:
			e.setAnim(crun.getAnim(exp[1].evalI(c), string(*(*[]byte)(unsafe.Pointer(&exp[0]))), false))
		case explod_angle:
